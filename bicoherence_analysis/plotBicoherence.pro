...
pro plotBicoherence, bicoherenceMatrix, plotParameters
  ...
  path2plot=startpath + strjoin(strtrim(string((bin_date(plotParameters.date))[0:2]),2),'-') + '/'
  ...
  device, filename=path2plot+plotName
    ...
    endelse
    plot,[min(freqAxis),plotParameters.maxFrequency],[min(freqAxis),yMaxFrequency],$
      xmargin=[10.5,4.5],ymargin=[10,3],$
      xstyle=1,ystyle=1,xticklen=-0.03,yticklen=-0.03,xthick=3,ythick=3,$
      charsize=2,charthick=3,$
      title=plotParameters.type+' from '+plotParameters.expName,xtitle='Frequency1 [kHz]',ytitle='Frequency2 [kHz]',$
      /nodata
